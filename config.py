# Trang VnExpress
vnexpress_rss_feeds = {
    'Thời sự': 'https://vnexpress.net/rss/thoi-su.rss',
    'Giải trí': 'https://vnexpress.net/rss/giai-tri.rss',
    'Pháp luật': 'https://vnexpress.net/rss/phap-luat.rss',
    'Sức khoẻ': 'https://vnexpress.net/rss/suc-khoe.rss',
    'Du Lịch': 'https://vnexpress.net/rss/du-lich.rss',
    'Xe': 'https://vnexpress.net/rss/oto-xe-may.rss',
    'Tâm sự': 'https://vnexpress.net/rss/tam-su.rss',
    'Thế giới': 'https://vnexpress.net/rss/the-gioi.rss',
    'Kinh doanh': 'https://vnexpress.net/rss/kinh-doanh.rss',
    'Thể thao': 'https://vnexpress.net/rss/the-thao.rss',
    'Đời sống': 'https://vnexpress.net/rss/gia-dinh.rss',
    'Khoa học công nghệ': 'https://vnexpress.net/rss/khoa-hoc-cong-nghe.rss',
    'Bất động sản': 'https://vnexpress.net/rss/bat-dong-san.rss'
}

# Trang Dân trí
dantri_rss_feeds = {
    'Giáo dục': 'https://dantri.com.vn/rss/giao-duc.rss',
    'Đời sống': 'https://dantri.com.vn/rss/doi-song.rss',
    'Tình yêu - Giới tính': 'https://dantri.com.vn/rss/tinh-yeu-gioi-tinh.rss',
    'Nội vụ': 'https://dantri.com.vn/rss/noi-vu.rss',
    'Tâm điểm': 'https://dantri.com.vn/rss/tam-diem.rss',
    'Sự kiện': 'https://dantri.com.vn/rss/su-kien.rss',
    'Lao động - Việc làm': 'https://dantri.com.vn/rss/lao-dong-viec-lam.rss',
    'Tấm lòng nhân ái': 'https://dantri.com.vn/rss/tam-long-nhan-ai.rss',
    'Thời tiết': 'https://dantri.com.vn/rss/thoi-tiet.rss'
}

# Trang Kênh 14
kenh14_rss_feeds = {
    'Star': 'https://kenh14.vn/star.rss',
    'Học đường': 'https://kenh14.vn/hoc-duong.rss',
    'Beauty & Fashion': 'https://kenh14.vn/beauty-fashion.rss',
    'Cine': 'https://kenh14.vn/cine.rss',
    'Musik': 'https://kenh14.vn/musik.rss',
    'Thế Giới Đó Đây': 'https://kenh14.vn/the-gioi-do-day.rss',
    'Đời sống': 'https://kenh14.vn/doi-song.rss',
    'Tek-life': 'https://kenh14.vn/tek-life.rss',
    'Money-Z': 'https://kenh14.vn/money-z.rss',
    'Sport': 'https://kenh14.vn/sport.rss',
    'Xã hội': 'https://kenh14.vn/xa-hoi.rss',
    'Ăn - Quẩy - Đi': 'https://kenh14.vn/an-quay-di.rss',
    'The 30s': 'https://kenh14.vn/the-30s.rss'
}

# Trang Afamily
afamily_rss_feeds = {
    'Lifestyle': 'https://afamily.vn/lifestyle.rss',
    'Đẹp': 'https://afamily.vn/dep.rss',
    'Mẹ và bé': 'https://afamily.vn/me-va-be.rss',
    'Ăn ngon': 'https://afamily.vn/an-ngon.rss',
    'Gia đình': 'https://afamily.vn/tam-su-gia-dinh.rss'
}

# Trang CafeBiz
cafebiz_rss_feeds = {
    'Kinh tế vĩ mô': 'https://cafebiz.vn/rss/vi-mo.rss',
    'Kinh doanh': 'https://cafebiz.vn/rss/cau-chuyen-kinh-doanh.rss',
    'Công nghệ': 'https://cafebiz.vn/rss/cong-nghe.rss',
    'Sống': 'https://cafebiz.vn/rss/song.rss',
    'Sức bật tư nhân': 'https://cafebiz.vn/rss/suc-bat-tu-nhan.rss',
    'BizMoney': 'https://cafebiz.vn/rss/bizmoney.rss',
}
